...
// データ取得SQL作成
$sql = "SELECT * FROM myon_table LEFT OUTER JOIN (SELECT todo_id, COUNT(id) AS cnt FROM like_table GROUP BY todo_id) AS likes ON myon_table.id = likes.todo_id";
...
if ($status == false) {
  ...
  foreach ($result as $record) {
    $output .= "<tr>";
    $output .= "<td>{$record["lng"]}</td>";
    $output .= "<td>{$record["lat"]}</td>";
